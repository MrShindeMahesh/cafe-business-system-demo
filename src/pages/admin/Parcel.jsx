...
import { ShoppingBag, Plus, FileText, Pencil, X } from 'lucide-react';
...
export default function Parcel() {
  ...
  // PARCEL BUGFIX: settle ONE ticket at a time — BillModal gets table.orderId so it
  // only grabs (marks SERVED) the ticket whose Bill/Settle button was clicked.
  const parcelOrders = orders.filter((o) => String(o.tableId || '').toUpperCase() === 'PARCEL' && o.status !== 'SERVED' && o.status !== 'CANCELLED');
          ...
          <div key={ord.id} className="card" style={{ padding: '1rem', border: '1px solid var(--color-border)' }}>
            ...
            <div style={{ display: 'flex', gap: '.4rem', marginTop: '.6rem', flexWrap: 'wrap' }}>
              {(role === 'admin' || role === 'reception') && (
                <button className="btn btn-primary btn-sm" style={{ flex: 1 }} onClick={() => setSelectedForBill({ id: 'PARCEL', number: 'PARCEL', orderId: ord.id })} title="Settle only this ticket">
              ...
              <button
                ...
                title="Cancel this parcel order"
                onClick={() => { if (window.confirm(`Cancel parcel order #${ord.id} (₹${ord.total})? The ticket will be removed from the open list.`)) updateOrderStatus(ord.id, 'CANCELLED'); }}
              >
