...
function buildKitchenTicket({ id, tableId, items = [], createdAt }) {
  ...
  let out = init + alignCenter + boldOn + big + cafe + normalSize + boldOff + '\n';
  out += 'KITCHEN ORDER TICKET\n';
  out += '==========================\n';
...
export async function printReceipt({ table, orders: tableOrders, settings: customSettings }) {
  ...
  let out = init + alignCenter + boldOn + cafe + boldOff + '\n';
  out += alignLeft + `Table: ${tNum}  ${now}\n`;
  out += `${tableOrders.length} ticket(s)\n`;
  out += '--------------------------------\n';
  ...
  for (const ord of tableOrders) {
    ...
    for (const it of (ord.items || [])) {
      out += `  ${(it.quantity || 1)}x ${it.name}\n`;
      if (it.customizationString) out += `    [${it.customizationString}]\n`;
    }
    out += `  Subtotal: Rs.${Number(ord.total || 0)}\n`;
    out += '--------------------------------\n';
  }

  out += alignCenter + boldOn + `TOTAL: Rs.${combinedTotal}` + boldOff + '\n';
  out += alignCenter + 'Thank you! Visit again :)\n';
  out += feed(3) + cut;

  if (!ip) {
    const dir = ensureLogDir();
    fs.appendFileSync(path.join(dir, 'print.log'), `[${new Date().toISOString()}] RECEIPT Table ${tNum} Rs.${combinedTotal}\n`);
    console.log('ℹ no printer configured — receipt logged to logs/print.log');
    return { logged: true };
  }

  await sendToPrinter(ip, Number(settings.printerPort || 9100), Buffer.from(out, 'latin1'));
  console.log(`✔ receipt Table ${tNum} sent to ${ip}`);
  return { ok: true };
}
