...
export default function Analytics() {
  ...
  const todaysPayments = useMemo(() => payments.filter((p) => p.settledAt && new Date(p.settledAt) >= dayStart), [payments, dayStart]);
  const paidOrderIds = useMemo(() => new Set(todaysPayments.flatMap((p) => p.orderIds || [])), [todaysPayments]);
  const todaysOrders = useMemo(() => orders.filter((o) => o.createdAt && new Date(o.createdAt) >= dayStart), [orders, dayStart]);
  ...
  const printDayClose = () => {
    ...
    rep.innerHTML =
      '<div style="text-align:center;margin-bottom:.6rem">' +
        ...
        '<div style="font-size:.75rem;color:#555">' + now + '</div>' +
      '</div>' +
    ...
    <div className="animate-fade-in">
      ...
      <div className="card" style={{ padding: '1.5rem', marginTop: '2rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <h3 style={{ fontFamily: 'var(--font-heading)', marginBottom: '.25rem' }}>Day Close (Z-Report)</h3>
            <p style={{ fontSize: '.8rem', color: 'var(--color-text-muted)' }}>
              Today: NET ₹{dayRevenue.toLocaleString()} · expenses ₹{dayExpenseTotal.toLocaleString()} → after expenses ₹{dayNetAfterExpenses.toLocaleString()} · {todaysPayments.length} bill(s) settled · {todaysOrders.length} order(s) placed · discounts ₹{dayDiscounts}
            </p>
          </div>
