...
// -------- migrate menu table to add veg / spiceLevel / costPrice (idempotent) --------
{
  const cols = db.prepare("PRAGMA table_info(menu)").all().map((r) => r.name);
  if (!cols.includes('veg')) db.exec("ALTER TABLE menu ADD COLUMN veg TEXT DEFAULT 'V'");
  if (!cols.includes('spiceLevel')) db.exec("ALTER TABLE menu ADD COLUMN spiceLevel TEXT DEFAULT 'Mild'");
  if (!cols.includes('costPrice')) db.exec("ALTER TABLE menu ADD COLUMN costPrice REAL DEFAULT 0");
}
...
if (menuCount === 0 || storedMenuVersion < MENU_SEED_VERSION) {
  const ins = db.prepare(
    'INSERT OR IGNORE INTO menu (id,name,category,price,description,image,available,recipe,variants,addons,veg,spiceLevel,costPrice) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)'
  );
  db.exec('BEGIN');
  try {
